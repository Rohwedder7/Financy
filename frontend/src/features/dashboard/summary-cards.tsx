...
export function SummaryCards({ dashboard }: { dashboard: DashboardTotals }) {
  ...
  return (
    <section aria-label="Resumo financeiro" className="grid gap-4 sm:grid-cols-3">
      {cards.map((card) => (
        <article className="rounded-2xl border border-financy-border bg-white px-4 py-4" key={card.label}>
          <h2 className="text-sm font-semibold uppercase tracking-wide text-financy-muted">{card.label}</h2>
          <p className={`mt-2 text-2xl font-semibold ${amountClass(card.label, card.amountInCents)}`}>
